...
int main()
{
	...
	Texture tSnow(R"(Textures\snow.jpg)");
	...
	glBindTexture(GL_TEXTURE_2D, texbl.id);
	glTexImage2D(
		GL_TEXTURE_2D, 0, GL_RGB16F, KAT_TEXTURE_SIZE, KAT_TEXTURE_SIZE, 0, GL_RGB, GL_FLOAT, NULL
	);
	...
	glFramebufferTexture2D(
		GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texbl.id, 0
	);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, hdrFBO);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		glViewport(0, 0, KAT_TEXTURE_SIZE, KAT_TEXTURE_SIZE);
		...
		glUniform1i(glGetUniformLocation(baseShader, "t_roughness"), 2);
